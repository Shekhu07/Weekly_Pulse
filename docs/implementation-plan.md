...
### Phase 8: Streamlit UI (Deprecated)
- *Note: This phase was superseded by Phase 9 to perfectly support the Google Stitch custom HTML layout.*
...
### Phase 9: FastAPI & Static UI Migration
- Migrated the application from Streamlit to a lightweight FastAPI server to support raw, beautifully structured HTML generated by Google Stitch.
- **Backend:** `api.py` exposes `/api/run` and `/api/ledger` to execute the Pulse pipeline and retrieve SQLite data, respectively.
- **Frontend:** `static/index.html` hosts the fully custom Tailwind-styled dashboard with Glassmorphism and Teal gradients, communicating with the backend via JavaScript fetch requests.
